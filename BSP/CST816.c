...
{
	return IIC_Read_One_Byte(&CST816_dev,Device_Addr,reg);
}
...
*********************************************************************************************************
*/
void CST816_IIC_WriteREG(uint8_t addr, uint8_t data)
{
	IIC_Write_One_Byte(&CST816_dev,Device_Addr,addr,data);
}
...
void CST816_Get_XY_AXIS(void)
{
    ...
    uint8_t DAT[4];  // 声明4字节数组，存储从触摸芯片读取的原始数据

    // 从触摸芯片的寄存器地址 `XposH` 开始，连续读取4字节数据到 DAT 数组
    IIC_Read_Multi_Byte(&CST816_dev, Device_Addr, XposH, 4, DAT);

    // 解析 X 坐标：DAT[0] 高4位 + DAT[1] 全8位 → 组合成12位坐标值
    CST816_Instance.X_Pos = ((DAT[0] & 0x0F) << 8) | DAT[1];

    // 解析 Y 坐标：DAT[2] 高4位 + DAT[3] 全8位 → 组合成12位坐标值，并加上垂直偏移
    CST816_Instance.Y_Pos = ((DAT[2] & 0x0F) << 8) | DAT[3] + TOUCH_OFFSET_Y;

    // 宏REVERSE 如果屏幕方向需要翻转
    #if REVERSE
        CST816_Instance.X_Pos = 239 - CST816_Instance.X_Pos;  // X轴反向（假设屏宽240像素）
        CST816_Instance.Y_Pos = 279 - CST816_Instance.Y_Pos;  // Y轴反向（假设屏高280像素）
    #endif
}
