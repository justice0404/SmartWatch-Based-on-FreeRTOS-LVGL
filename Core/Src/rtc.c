...
void MX_RTC_Init(void)
{
  ...
  */
  ...
  /* USER CODE BEGIN Check_RTC_BKUP */
	if(HAL_RTCEx_BKUPRead(&hrtc,RTC_BKP_DR0)!=0x5050) //是否为第一次配置
	{
...
uint8_t weekday_cauculate(int y,int m,int d,int c)
{
	int w;
	if (m == 1 || m == 2)
	{y--, m += 12;}
...
void RTC_SetDate(uint8_t year,uint8_t month,uint8_t date)
{
	...
	setdate.WeekDay = weekday_cauculate(year,month,date,20);
